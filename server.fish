...
while true
    ...
    if echo $body | jq 'has("blame")' | grep -q true
        echo "received blame:"
        echo $body
        curl -d "$body" -H "Content-Type: application/json" -X POST $blame || echo "failed to blame"
    else
        if echo $body | jq 'has("id")' | grep -q true
            ...
            if grep -Fxq "$id" $ids_file
                echo "Received again, loop completed!"
            else
                echo "$id" >>$ids_file
            ...
            '{id: $id, author: $author, message: $message, relayers: $relayers}')
                ...
                # Send a POST request
                # post to tony
                if ! curl -d "$body" -H "Content-Type: application/json" -X POST $relay_to
                    echo "failed to send, blaming tony back to Kars"
                    curl -d '{"blame": "Tony"}' -H "Content-Type: application/json" -X POST $blame || echo "failed to blame"
                end
            end
        else
